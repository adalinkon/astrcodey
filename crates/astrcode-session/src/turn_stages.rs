...
use crate::mcp_visibility::{clone_tools_by_index, provider_visible_tool_indexes};
...
impl TurnState {
    pub(crate) fn new(
        ...
        let active_mcp_tools = HashSet::new();
        let tool_indexes = provider_visible_tool_indexes(&all_tools, &active_mcp_tools);
    ...
    pub(crate) fn all_tools(&self) -> &[ToolDefinition] {
        &self.all_tools
    }

    pub(crate) fn visible_tools(&self) -> &[ToolDefinition] {
        &self.visible_tools
    }

    pub(crate) fn active_mcp_tools(&self) -> &HashSet<String> {
        &self.active_mcp_tools
    }

    pub(crate) fn activate_mcp_tools(&mut self, discovered_tools: Vec<String>) -> bool {
        let changed = crate::mcp_visibility::activate_discovered_mcp_tools(
            &mut self.active_mcp_tools,
            &self.all_tools,
            discovered_tools,
        );
        if changed {
            let tool_indexes =
                provider_visible_tool_indexes(&self.all_tools, &self.active_mcp_tools);
