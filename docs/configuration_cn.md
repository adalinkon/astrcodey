...
WASM 扩展在 `extension.json` 中声明所需宿主能力；未声明的敏感能力不会注入运行上下文：
...
可声明能力包括 `session_state`、`session_control`、`small_model`、`session_history`、`emit_events`、`workspace_read`、`process_spawn` 和 `network_client`。
