...
## Continuations（多段后台管线）

WASM guest 可在 [`CallResponse`](./plugin-system-wasm-s6r.md) 中返回 `continuations`：宿主在当前 `extension_call` 完成后**顺序**调度 follow-up（深度上限 16）。NonBlocking hook 内适用，用于 memory 类多步提取而无需 sidecar。
