...
AstrCode 是一个 Rust 原生 AI 编程助手，当前主路径是浏览器 Web 前端和 HTTP/SSE server。包含带工具执行的 Agent 循环、基于 SSE 流式传输的多 Provider LLM 层（Anthropic、OpenAI、Google GenAI）、基于 SDK 与 IPC 子进程的扩展/钩子系统（后台预热、健康检查、启动阶段事件通道）、MCP 常驻进程池（跨 turn 复用长连接）、内置 Web 搜索与 URL 抓取工具、带自动压缩的上下文窗口管理、评测框架，以及用于编辑器集成的 ACP（Agent Client Protocol）适配器。
...
按[快速开始](#快速开始)从源代码构建。预编译 server 二进制通过 GitHub Releases 发布。
          ...
          └────────┬──────────────┘  └─────┬─────┘
                   ...
                   └────────┬──────────────┘
                        ...
                        └───┬────────┘
                            │
                   ┌────────┴────────┐
                   │astrcode-session │  Agent 循环核心：turn runner、工具管线
                   │                 │  LLM 流消费、上下文压缩编排
                   └────────┬────────┘
             ┌───────────┼───────────┐
             │           │           │
...
Cargo workspace 包含 [`crates/`](crates/) 下的 server/runtime crate，以及以 [`eval-tasks/`](eval-tasks) 为根的评测 crate。浏览器前端位于 [`frontend/`](frontend)。按架构分层如下（详见[架构设计](docs/architecture.md)）。
...
| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-eval`](eval-tasks) | 1.0k | 评测运行器：HTTP 服务器控制、事件日志指标、结构化报告 |

**合计：** Rust workspace crate 加上 [`frontend/`](frontend) 中的 React + TypeScript 前端。
...
| 模式 | 命令 | 说明 |
|---|---|---|
| **Server** | `cargo run -p astrcode-server --bin astrcode-http-server` | HTTP/SSE 服务器，支持 JSON-RPC、会话管理、实时事件流 |
| **ACP** | `cargo run -- acp` | ACP stdio 适配器，用于 IDE/编辑器集成 |
| **Eval** | `cargo run -p astrcode-eval -- --cases eval-tasks/cases` | 运行评测基准 |
...
每个版本标签自动触发 GitHub Release，提供 Linux、macOS、Windows（x86_64 + aarch64）的预编译 server 二进制文件。每周一自动发布 patch 版本。
...
- **[Codex](https://github.com/openai/codex)** — Agent 交互和编程工作流启发了 AstrCode 的部分体验设计。
