...
AstrCode 是一个全栈 AI 编程助手，用约 5.7 万行 Rust 代码、20 个 crate 构建，外加 React + TypeScript 前端（约 4800 行）。包含带工具执行的 Agent 循环、基于 SSE 流式传输的多 Provider LLM 层（Anthropic、OpenAI、Google GenAI）、扩展/钩子系统（支持 FFI 加载原生扩展和 WASM 扩展、后台预热、健康检查、启动阶段事件通道）、MCP 常驻进程池（跨 turn 复用长连接）、带自动压缩的上下文窗口管理、评测框架，以及多种交互方式：终端 TUI、Web 前端、Tauri 桌面应用、HTTP/SSE API 和 ACP（Agent Client Protocol）适配器。
...
- **扩展运行时 API** — `Extension::start()`（携带 `ExtensionCtx`，含 `startup_working_dir` 和 `event_sink`）、`Extension::stop()`（携带 `StopReason`）、`Extension::health()`（健康探测）、`Extension::on_config_changed()`（热更新配置）
