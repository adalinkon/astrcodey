...
`astrcode` npm 包（v0.1.3）提供了 Linux、macOS 和 Windows（x64 + arm64）的预编译二进制文件。安装后，`astrcode` 命令将全局可用。
...
**扩展配置**：可通过 `~/.astrcode/config.json` 中的 `extensionStates` 字段启用或禁用扩展。默认情况下，除 `memory` 扩展外，所有扩展均处于启用状态。`memory` 扩展默认关闭。
