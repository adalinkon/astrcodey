...
AstrCode 是一个全栈 AI 编程助手，用约 5.4 万行 Rust 代码、19 个 crate 构建，外加 React + TypeScript 前端（约 4800 行）。包含带工具执行的 Agent 循环、基于 SSE 流式传输的 LLM Provider 层、插件/钩子扩展系统（支持通过 FFI 加载原生扩展和 WASM 扩展）、带自动压缩的上下文窗口管理，以及多种交互方式：终端 TUI、Web 前端、Tauri 桌面应用、HTTP/SSE API 和 ACP（Agent Client Protocol）适配器。
...
| Crate | 行数 | 说明 |
|---|---|---|
| `astrcode-server` | 9.3k | Agent 循环、会话管理、JSON-RPC/HTTP/ACP 处理器、transport、并发控制 |
| `astrcode-cli` | 8.2k | 终端 TUI（ratatui）、无头执行、服务器启动器 |
| `astrcode-session` | 5.2k | 会话运行时：会话句柄、Turn 执行、工具管线、事件总线 |
| `astrcode-tools` | 4.6k | 内置工具：read、write、edit、patch、find、grep、shell、task |
| `astrcode-core` | 4.7k | 共享类型、trait、配置系统、错误类型、提示词组合、扩展契约 |
| `astrcode-ai` | 3.6k | OpenAI 兼容 Provider（Chat Completions + Responses API）、SSE 流式、重试 |
| `astrcode-context` | 3.5k | Token 估算、上下文窗口预算、自动压缩、提示词引擎 |
| `astrcode-storage` | 3.5k | JSONL 事件日志、会话快照、配置持久化、文件锁 |
| `astrcode-extensions` | 2.9k | 扩展生命周期、钩子分发、原生扩展加载（FFI）、WASM 扩展运行时 |
...
| `astrcode-support` | 635 | 路径解析、Shell 检测、文本处理 |
...
**共计：19 个 crate、192 个源文件、约 5.4 万行代码。**
...
NPM 包（`@anthropics/astrcode`）同步发布到 npm registry。
